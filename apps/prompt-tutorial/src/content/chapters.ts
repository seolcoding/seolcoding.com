...
export interface Exercise {
  ...
  hints: string[];
  // LLM 기반 채점 - 모든 연습문제가 LLM 채점 사용
  useLLMGrading: true;
  // 채점 기준 (한국어)
  grading: {
    target: 'response' | 'input';
    type: 'llm'; // 모든 채점은 LLM 기반
    criteria: string; // 한국어 채점 기준
  };
}
...
AI와 대화할 때는 **Messages API** 형식을 사용합니다.
...
각 메시지는 \`role\`(역할)과 \`content\`(내용)로 구성됩니다. \`user\` 역할 메시지 하나만 보낼 수도 있고, \`user\`와 \`assistant\` 메시지를 번갈아 여러 개 보낼 수도 있습니다. **첫 번째 메시지는 항상 user 역할이어야 합니다.**

### 시스템 프롬프트

**시스템 프롬프트**를 사용하면 AI에게 **맥락, 지침, 가이드라인을 미리 제공**할 수 있습니다.
    ...
    exercises: [
      {
        ...
        defaultUserPrompt: '',
        hints: [
          'AI에게 간단하게 숫자를 세어달라고 요청해보세요. 예: "1부터 3까지 세어줘"',
          'AI는 순서대로 세는 방법을 알고 있습니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 1, 2, 3을 순서대로 포함하고 있는지 확인하세요. 응답이 "1, 2, 3" 또는 "하나, 둘, 셋" 또는 "첫째, 둘째, 셋째" 등으로 셋까지 순서대로 세고 있으면 정답입니다. 응답에 불필요한 설명이 있어도 숫자 세기가 올바르면 정답으로 인정하세요.'
        }
      },
      {
        id: '1.2',
        ...
        hints: [
          '시스템 프롬프트에 "너는 3살 아이야" 또는 "유치하게 말해줘"라고 지시해보세요.',
          '3살 아이처럼 간단한 단어, 반복, 애교 섞인 말투를 사용하는지 확인하세요.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 3살 아이의 말투로 작성되었는지 확인하세요. 간단한 단어, 반복적인 표현, 애교 섞인 말투, "~해", "~야" 등의 어미 사용, 귀여운 표현 등이 있으면 정답입니다. 예: "하늘은 엄청 커! 정말 정말 크다!"'
        }
      }
    ]
  },
...
- 프롬프트를 동료나 친구에게 보여주고 직접 따라하게 해보세요. 그들이 헷갈려하면, AI도 헷갈려합니다.
    ...
    examples: [
      {
        userPrompt: '로봇에 대한 짧은 시를 써주세요.',
        description: '단순 요청 - AI가 서론을 붙일 수 있음'
      },
      {
        userPrompt: '로봇에 대한 짧은 시를 써주세요. 서론 없이 바로 시만 작성해주세요.',
    ...
    exercises: [
      {
        ...
        hints: [
          '시스템 프롬프트에 "스페인어로 답변해줘"라고 지시해보세요.',
          '응답의 대부분이 스페인어 단어로 구성되어 있어야 합니다. hola, gracias, bien 등'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답의 주요 내용이 스페인어로 작성되었는지 확인하세요. 인사말(hola, holas 등), 스페인어 기본 단어(gracias, bien, 등)가 포함되어 있고 응답 전체가 스페인어 어조면 정답입니다. 한국어와 섞여 있어도 스페인어가 주를 이루면 정답으로 인정하세요.'
        }
      },
      {
        id: '2.2',
        ...
        defaultUserPrompt: '',
        hints: [
          'AI에게 "이름만 답해줘", "다른 말 없이 이름만", "오직 이름만 출력" 등으로 지시해보세요.',
          '응답이 정확히 "Michael Jordan"이어야 합니다. 다른 텍스트가 없어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 정확히 "Michael Jordan"이어야 합니다. 대소문자는 상관없습니다. 응답에 다른 단어, 구두점, 설명이 있으면 오답입니다. 공백만 허용됩니다. 예: "Michael Jordan" ✓, "Michael Jordan입니다" ✗'
        }
      },
      {
        id: '2.3',
        ...
        defaultUserPrompt: '',
        hints: [
          'AI에게 "800단어 이상으로 길게 작성해줘", "최대한 상세하게", "아주 길게" 등으로 지시해보세요.',
          '응답이 충분히 긴지 확인하세요. 800단어 이상이면 정답입니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답의 길이를 확인하세요. 응답이 800단어 이상이면 정답입니다. 응답이 짧으면(200단어 미만) 오답입니다. 중간 길이(200-799단어)는 부분 점수를 부여하세요. 내용이 일관성 있고 의미 있는 긴 이야기인지도 확인하세요.'
        }
      }
    ]
  },
...
x = 3`,
        hints: [
          'AI에게 수학 문제를 더 잘 풀 수 있는 역할을 부여해보세요. 예: "당신은 수학 교사야" 또는 "당신은 꼼꼼한 계산가야"',
          '또는 AI에게 "단계별로 확인하면서 풀어줘"라고 지시해보세요.',
          '응답에 "틀렸다", "오류", "incorrect", "wrong" 등이 포함되어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 이 문제 풀이가 틀렸다고 평가하는지 확인하세요. 2단계(2x = 6)가 잘못되었다는 것을 지적하고 "틀렸다", "오류", "incorrect", "wrong", "잘못되었다" 등으로 평가하면 정답입니다. 문제가 올바르다고 평가하면 오답입니다.'
        }
      }
    ]
  },
    ...
    exercises: [
      {
        id: '4.1',
        title: '짧은 시 주제',
        description: '프롬프트를 수정해서 돼지(pig)에 대한 짧은 시를 출력하도록 만들어 보세요.',
        systemPromptEditable: false,
        userPromptEditable: true,
        defaultSystemPrompt: '',
        defaultUserPrompt: '',
        hints: [
          'AI에게 "돼지에 대한 짧은 시를 써줘"라고 요청해보세요.',
          '짧은 시는 3행으로 구성된 간결한 시 형식입니다. 5-7-5 음절 구조가 전통적입니다.',
          '응답이 돼지에 대한 내용을 담은 짧은 시 형식이어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 돼지에 대한 짧은 시인지 확인하세요. 응답이 3행으로 구성되어 있고, 돼지(pig)나 돼지 관련 키워드가 포함되어 있으며, 짧은 시의 특징(간결한 시, 5-7-5 음절 구조 등)을 보이면 정답입니다. "돼지에 대해 설명하는 글"처럼 긴 설명이면 오답입니다. 응답이 "돼지" 단어만 포함하지만 실제 짧은 시 형식이 아니면 부분 점수를 주세요.'
        }
      },
      {
        id: '4.2',
...
다른 것은 바꾸지 마세요. 지저분하고 실수투성이인 글은 의도적인 것입니다. AI가 이런 실수에 어떻게 반응하는지 보기 위함입니다.`,
        ...
        hints: [
          '질문 부분을 XML 태그로 감싸보세요. 예: <question>...</question>',
          '응답에 "brown"이 포함되어 있어야 합니다. 개 털색이 갈색(brown)이라는 것을 맞혀야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 "brown" 또는 "갈색"이 포함되어 있고, 개(dog)의 털색이 갈색이라는 내용을 올바르게 답변하고 있으면 정답입니다. XML 태그를 사용하여 질문과 지저분한 텍스트를 분리했더라 올바른 답변을 하는지 확인하세요.'
        }
      },
      {
        id: '4.3',
...
위 연습문제와 마찬가지로 다른 것은 바꾸지 마세요. 이렇게 하면 AI가 어떤 종류의 언어를 파싱하고 이해할 수 있는지 볼 수 있습니다.`,
        ...
        defaultUserPrompt: 'Hia its me i have a q about dogs jkaerjv ar cn brown? jklmvca tx it help me muhch much atx fst fst answer short short tx',
        hints: [
          '지저분하고 의미 없는 텍스트 조각들을 하나씩 제거해보세요.',
          '핵심 질문("dogs brown?")만 남겼을 때 AI가 올바르게 답하는지 확인하세요.',
          '응답에 "brown"이 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 "brown" 또는 "갈색"이 포함되어 있고, 개(dog)의 털색이 갈색이라는 내용을 올바르게 답변하고 있으면 정답입니다. 프롬프트에서 지저분한 텍스트를 제거하여 질문을 명확하게 만들었더라 올바른 답변을 하는지 확인하세요.'
        }
      }
    ]
  },
...
더 나아가서, **\`assistant\` 턴에 첫 번째 XML 태그를 넣을 수 있습니다.** \`assistant\` 턴에 텍스트를 넣으면, 기본적으로 AI에게 이미 뭔가를 말했다고 알려주고 그 지점부터 계속하도록 하는 것입니다. 이 기법을 **"AI 대신 말하기" 또는 "응답 미리 채우기(prefilling)"**이라고 합니다.
...
API를 통해 AI를 호출할 때, \`stop_sequences\` 매개변수에 닫는 XML 태그를 전달하여 원하는 태그를 출력한 후 AI가 샘플링을 멈추도록 할 수 있습니다. 이미 원하는 답변을 받은 후의 AI의 마무리 발언을 제거하여 비용과 시간을 절약할 수 있습니다.
    ...
    examples: [
      {
        userPrompt: '토끼에 대한 짧은 시를 써주세요. <시> 태그 안에 넣어주세요.',
        description: 'XML 태그로 출력 요청 - AI가 서론을 붙일 수 있음'
      },
      {
        systemPrompt: '',
        userPrompt: '고양이에 대한 짧은 시를 써주세요. <시> 태그 안에 넣어주세요.',
        description: 'Prefill 사용 - assistant 응답을 "<시>"로 시작하여 AI가 바로 시를 작성하게 함'
      },
      {
        userPrompt: '고양이에 대한 짧은 시를 써주세요. JSON 형식으로 작성하되, 키는 "first_line", "second_line", "third_line"으로 해주세요.',
...
힌트: AI의 응답을 시작하는 부분을 프롬프트에 포함시켜 보세요.`,
        ...
        hints: [
          '응답 미리 채우기(prefill)를 사용해보세요. assistant 응답을 "스테판 커리가 최고인 이유는"으로 시작하게 하세요.',
          '응답에 "스테판 커리"나 "커리"가 포함되어 있고, 그가 워리어스(Warriors) 소속임을 언급하면 정답입니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 스테판 커리(Steph Curry)를 역대 최고의 농구 선수로 주장하고 있으면 정답입니다. "커리", "스테판 커리", "Steph Curry", "Curry" 등의 이름이 포함되어 있고, 그가 골든스테이트 워리어스(Golden State Warriors) 소속임을 언급하면 완벽합니다. 마이클 조던 대신 커리를 선택한 이유를 논거로 설명하고 있으면 정답으로 인정하세요.'
        }
      },
      {
        id: '5.2',
        title: '두 개의 짧은 시',
        description: 'XML 태그를 사용하여 프롬프트를 수정해서 AI가 동물에 대한 짧은 시를 하나가 아니라 두 개 작성하도록 만들어 보세요. 한 시가 끝나고 다른 시가 시작하는 것이 명확해야 합니다.',
        systemPromptEditable: false,
        userPromptEditable: true,
        defaultSystemPrompt: '',
        defaultUserPrompt: '고양이에 대한 짧은 시를 써주세요. <시> 태그 안에 넣어주세요.',
        hints: [
          '<시> 태그를 두 번 사용해보세요.',
          '각 짧은 시가 <시>...</시>로 감싸져 있어야 합니다.',
          '응답에 고양이(cat/고양이)가 언급되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 <시> 태그가 두 번 이상 나타나고, 각 태그 안에 짧은 시 내용이 들어있으면 정답입니다. 응답 전체에 고양이(cat/고양이) 관련 내용이 포함되어 있어야 합니다. <시> 태그가 명확히 구분되어 두 개의 짧은 시임을 알 수 있어야 합니다.'
        }
      },
      {
        id: '5.3',
        title: '두 개의 짧은 시, 두 마리의 동물',
        description: '프롬프트를 수정해서 **AI가 두 마리의 다른 동물에 대한 짧은 시를 두 개 작성하도록** 만들어 보세요. 고양이와 꼬리(tail)가 있는 다른 동물에 대해 작성하세요.',
        systemPromptEditable: false,
        userPromptEditable: true,
        defaultSystemPrompt: '',
        defaultUserPrompt: '고양이에 대한 짧은 시를 써주세요. <시> 태그 안에 넣어주세요.',
        hints: [
          '두 마리의 다른 동물을 요청하세요: 고양이와 꼬리가 있는 다른 동물(예: 강아지, 여우, 말 등)',
          '<시> 태그를 두 번 사용하고, 각각 다른 동물에 대한 짧은 시가 들어있어야 합니다.',
          '응답에 고양이(cat/고양이)와 꼬리(tail/꼬리)가 모두 언급되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 <시> 태그가 두 번 이상 나타나고, 두 마리의 다른 동물(하나는 고양이, 다른 하나는 꼬리가 있는 동물)에 대한 짧은 시가 작성되어 있으면 정답입니다. 응답에 "고양이"(cat/고양이)와 "꼬리"(tail/꼬리)가 모두 포함되어 있어야 합니다. 두 짧은 시가 명확히 구분되어야 합니다.'
        }
      }
    ]
  },
...
테스트 이메일: "안녕하세요 -- 제 Mixmaster4000이 작동할 때 이상한 소리가 납니다. 타는 전자기기 같은 연기 냄새와 플라스틱 냄새도 좀 납니다. 교체품이 필요합니다."`,
        ...
        hints: [
          '카테고리 목록을 프롬프트에 포함시키고, 분류 결과만 출력하도록 지시하세요.',
          '정답은 (B) 고장 또는 결함 제품입니다.',
          '응답에 "B", "고장", "결함"이 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 이 이메일을 (B) "고장 또는 결함 제품"으로 올바르게 분류하고 있으면 정답입니다. 응답에 "B", "고장", "결함", "Broken", "defective" 등이 포함되어 있어야 합니다. Mixmaster4000이 고장 났다는 것을 올바르게 파악했으면 정답으로 인정하세요.'
        }
      },
      {
        id: '6.2',
...
테스트 이메일: "4개월 동안 해지 후에도 월 요금이 계속 청구되고 있어요!! 대체 무슨 일이에요???"`,
        ...
        hints: [
          '<answer> 태그를 사용하여 응답을 감싸도록 지시하세요.',
          '정답은 <answer>C</answer> 형식이어야 합니다. (결제 관련 질문)',
          '응답에 정확히 "<answer>C</answer>"가 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 정확히 "<answer>C</answer>" 문자열이 포함되어 있으면 정답입니다. 결제 관련 질문이므로 (C)가 올바른 분류입니다. 응답 형식이 정확히 XML 태그로 감싸져 있어야 합니다.'
        }
      }
    ]
  },
...
2. **톤과 스타일**: 원하는 어조나 말투를 자연스럽게 따릅니다
...
테스트 이메일: "Mixmaster 4000으로 페인트를 섞을 수 있나요, 아니면 음식만 섞을 수 있나요?"`,
        ...
        hints: [
          '프롬프트에 2개 이상의 예제 이메일과 그 분류를 포함하세요.',
          '예제: "제품이 고장났어요" -> (B), "환불해주세요" -> (C)',
          '응답의 마지막 글자가 A 또는 D(구매 전 질문 또는 기타)이어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답의 마지막 글자가 A 또는 D이면 정답입니다. Mixmaster 4000으로 페인트를 섞을 수 있는지 묻는 질문이므로 "구매 전 질문"(A) 또는 "기타"(D)가 올바른 분류입니다. 응답이 예제 형식을 따르고, 마지막에 분류 문자(A, B, C, D 중 하나)로 끝나면 정답으로 인정하세요.'
        }
      }
    ]
  },
...
하지만 **환각을 피하는 방법은 많습니다**. 이 과정에서 이미 배운 많은 기법들을 포함해서요. AI가 환각을 일으키면, 여러 기법을 실험해서 정확도를 높여보세요.

### 핵심 기법

1. **탈출구 제공**: AI에게 "모르겠습니다"라고 말할 수 있도록 허용
2. **증거 기반 답변**: 답변하기 전에 관련 인용문을 먼저 찾도록 요청
3. **확신도 요청**: AI에게 답변의 확신도를 표시하도록 요청
4. **출처 명시**: 정보의 출처를 밝히도록 요청

### 왜 AI가 환각을 일으키나?

AI는 **도움이 되려고 노력합니다**. 때로는 이 욕구가 너무 강해서, 실제로 모르는 것도 답하려고 합니다. "탈출구"를 제공하면 AI가 모를 때 솔직하게 말할 수 있습니다.
    ...
    exercises: [
      {
        id: '8.1',
        title: '비욘세 환각 고치기',
        description: `AI의 환각 문제를 탈출구를 제공하여 고쳐보세요.

참고: Renaissance는 비욘세의 일곱 번째 정규 앨범이지, 여덟 번째가 아닙니다. AI가 모른다고 인정하도록 프롬프트를 수정하세요.`,
        systemPromptEditable: false,
        userPromptEditable: true,
        defaultSystemPrompt: '',
        defaultUserPrompt: '스타 가수 비욘세는 몇 년도에 여덟 번째 정규 앨범을 발매햀나요?',
        hints: [
          'AI에게 "모르면 모른다고 해줘", "확실하지 않으면 말해줘" 등으로 탈출구를 제공하세요.',
          '비욘세의 여덟 번째 앨범이 없다는 것을 AI가 인정하면 정답입니다.',
          '응답에 "모르겠습니다", "알 수 없습니다", "확실하지 않습니다" 등이 포함되어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 비욘세의 여덟 번째 앨범 발매 연도를 모른다고 인정하면 정답입니다. "모르겠습니다", "알 수 없습니다", "확실하지 않습니다", "I don\'t know", "not sure" 등의 표현이 포함되어 있어야 합니다. Renaissance가 일곱 번째 앨범이라는 사실을 언급하며 여덟 번째 앨범이 없다고 설명하면 완벽합니다.'
        }
      },
      {
        id: '8.2',
        title: '인용문으로 환각 방지',
        description: `프롬프트를 수정해서 AI가 인용문을 먼저 찾도록 하여 환각을 방지하세요.
...
2018년 12월에 약 12만 5천 명의 구독자로 시작하여, 2022년 12월에는 약 613만 명으로 구독자 기반이 약 49배 증가했습니다. 이 성장은 주로 부동산 분야에서의 3D 투어 수요 증가와 팬데믹 기간 동안의 가상 투어 필요성에 기인합니다.
        ...
        hints: [
          'AI에게 먼저 문서에서 관련 인용문을 찾도록 요청하세요.',
          '응답에 "49배" 또는 "49-fold"가 포함되어 있어야 합니다.',
          '인용문을 사용하여 문서의 내용을 정확히 반영하고 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 "49배" 또는 "49-fold" 또는 "49 times"가 포함되어 있으면 정답입니다. 문서의 내용을 정확히 반영하고 있어야 합니다. 인용문을 먼저 찾도록 요청했더라 정확한 답변을 하는지 확인하세요. 구독자가 49배 성장했다는 사실을 맞히면 정답입니다.'
        }
      }
    ]
  },
...
축하합니다! 마지막 챕터에 도달했습니다. 이제 모든 것을 종합해서 **고유하고 복잡한 프롬프트를 만드는 방법**을 배워봅시다.

아래에서 **복잡한 프롬프트에 권장되는 구조**를 사용할 것입니다. 이 챕터의 후반부에서는 산업별 프롬프트 예제와 그 구조를 설명합니다.

**참고:** **모든 프롬프트가 다음의 모든 요소를 필요로 하는 것은 아닙니다**. 요소를 포함하거나 제외해보면서 AI의 응답에 어떤 영향을 미치는지 실험해보세요. 보통 **먼저 많은 프롬프트 요소를 사용하여 작동시킨 후, 나중에 정제하고 간소화**하는 것이 좋습니다.

### 복잡한 프롬프트의 구조 (순서대로)

1. **사용자 역할**: Messages API 호출은 항상 \`user\` 역할로 시작해야 합니다
2. **작업 맥락**: AI가 맡아야 할 역할이나 목표에 대한 맥락 (프롬프트 초반에 배치)
3. **톤 맥락**: AI가 사용해야 할 어조 (필요한 경우)
...
8. **사전 인지**: 답변하기 전에 단계별로 생각하도록 요청
9. **출력 포맷팅**: 원하는 출력 형식 명시
10. **응답 미리 채우기**: AI 응답을 특정 텍스트로 시작하도록 유도

### 핵심 원칙

- **순서가 중요합니다** (일부 요소에서)
- **명확한 구분**: XML 태그로 섹션을 구분
- **탈출구 제공**: AI가 모를 때 어떻게 해야 하는지 명시
...
먼저 관련 인용문을 <quotes> 태그에 추출한 다음 답변해주세요.`,
        hints: [
          '복잡한 프롬프트 요소들을 활용하세요: 역할, 입력 데이터(XML 태그), 사전 인지(인용문 먼저), 출력 형식.',
          '정답은 "30일"입니다.',
          '응답에 "30일"이 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 "30일" 또는 "30 days"가 포함되어 있으면 정답입니다. 세금 코드에서 "해당 양도일로부터 30일 이내에 이루어져야 합니다"라는 내용을 정확히 찾아내고 있어야 합니다. 인용문을 먼저 추출하도록 요청했더라 정확한 답변을 하는지 확인하세요.'
        }
      },
      {
        id: '9.2',
...
<issue> 태그에 문제점을, <response> 태그에 소크라테스식 안내를 넣어주세요.`,
        hints: [
          'AI에게 소크라테스식 튜터 역할을 부여하고, 답을 직접 주지 않고 힌트만 제공하도록 하세요.',
          '코드의 문제는 i=0일 때 0으로 나누기(division by zero) 오류가 발생하는 것입니다.',
          '응답에 "0으로 나누기", "division by zero", "range", "ZeroDivision" 등이 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 코드의 문제점을 식별하고 있으면 정답입니다. "0으로 나누기", "division by zero", "ZeroDivision", "range(0)부터 시작" 등의 문제를 언급하고 있어야 합니다. 중요한 것은 정답을 직접 알려주지 않고 힌트만 제공하여 사용자가 스스로 찾도록 유도하는 소크라테스식 접근인지 확인하세요. <issue>와 <response> 태그를 사용하고 있는지도 확인하세요.'
        }
      }
    ]
  },
...
먼저 <names> 태그에 찾은 이름들을 나열하고, 그 다음 <sorted> 태그에 알파벳 순으로 정렬된 결과를 출력하세요.`,
        hints: [
          ...
          '추출된 이름: 제시, 나야, 에린, 조이, 케이샤, 멜',
          '알파벳 순 정렬: 에린, 멜, 제시, 조이, 케이샤, 나야 (Erin, Jesse, Joey, Keisha, Mel, Naya)',
          '응답에 이름들이 올바른 순서로 정렬되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에서 이름들이 알파벳 순서로 정렬되어 있으면 정답입니다. "에린, 멜, 제시, 조이, 케이샤, 나야" 또는 "Erin, Jesse, Joey, Keisha, Mel, Naya" 순서로 나타나면 정답입니다. <names>와 <sorted> 태그를 사용하여 단계별로 처리했는지 확인하세요. 모든 이름이 포함되어 있고 알파벳 순서가 올바르면 정답으로 인정하세요.'
        }
      }
    ]
  },
...
처음에는 개념적으로 복잡해 보일 수 있지만, **도구 사용(tool use)** 또는 **함수 호출(function calling)**은 실제로 매우 간단합니다! 이미 도구 사용을 구현하는 데 필요한 모든 기술을 알고 있습니다. 도구 사용은 실제로 **치환**과 **프롬프트 체이닝**의 조합일 뿐입니다.

### 도구 사용이란?

AI는 실제로 도구나 함수를 직접 호출할 수 없습니다. 대신 다음과 같은 방식으로 작동합니다:

1. **도구 호출 출력**: AI가 호출하려는 도구 이름과 매개변수를 출력
2. **응답 생성 중단**: 도구가 호출되는 동안 추가 응답 생성 중단
3. **결과로 다시 프롬프트**: 도구 결과를 추가하여 다시 프롬프트
...
- **계산기**: 정확한 수학 연산
- **데이터베이스 쿼리**: SQL 데이터베이스 조회 및 데이터 검색
- **API 호출**: 날씨 API, 검색 API 등
- **파일 처리**: 파일 읽기/쓰기
...
<parameters>
  <parameter>
    ...
    <type>int</type>
    <description>첫 번째 피연산자</description>
  </parameter>
  <parameter>
    <name>operator</name>
    <type>str</type>
...
도구를 호출하려면 <function_call> 태그를 사용하세요.`,
        ...
        hints: [
          '시스템 프롬프트에 get_user, get_product, add_user, add_product 4가지 도구를 정의하세요.',
          '각 도구의 이름, 설명, 매개변수를 명확히 작성하세요.',
          '응답에 get_user 도구 호출과 user_id=2가 포함되어 있어야 합니다.',
          '<function_call><name>get_user</name><parameters><user_id>2</user_id>...</parameters></function_call> 형식이어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답이 "get_user" 도구를 호출하고 user_id로 2를 사용하는 형식이면 정답입니다. <function_call> 태그 안에 <name>get_user</name>이 있고, <parameters> 안에 <user_id>2</user_id>가 있어야 합니다. 올바른 XML 태그 형식을 사용하고 있는지 확인하세요.'
        }
      }
    ]
  },
...
RAG 구현에 관심이 있다면:
- 벡터 데이터베이스 (Pinecone, Weaviate, ChromaDB 등)
- 임베딩 모델
- 청킹 전략 (문서를 적절한 크기로 나누기)
...
먼저 <relevant_doc> 태그에 관련 문서 ID를 표시하고, <quotes> 태그에 관련 인용문을 추출한 다음, <answer> 태그에 답변해주세요.`,
        hints: [
          ...
          '정답은 "귀도 반 로섬" 또는 "Guido van Rossum"입니다.',
          '응답에 창시자 이름이 포함되어 있어야 합니다.'
        ],
        useLLMGrading: true,
        grading: {
          target: 'response',
          type: 'llm',
          criteria: '응답에 "귀도" 또는 "Guido"가 포함되어 있고, 문서에서 인용문을 추출하여 답변하고 있으면 정답입니다. <relevant_doc>, <quotes>, <answer> 태그를 사용하고 있는지 확인하세요. 문서 1(Python 소개)을 참조하여 "Guido van Rossum" 또는 "귀도 반 로섬"이 창시자임을 올바르게 답변하면 정답으로 인정하세요.'
        }
      }
    ]
  }
